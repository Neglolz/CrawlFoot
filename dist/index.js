...
var dataLinkPlayer = [[id = String], [nom = String], [link = String]];
    ...
    c.queue([{
        ...
        callback: function callback(err, res, done) {
            if (err) {
                ...
                var playerPerPage = $("#no-more-tables > table > tbody > tr").length;
                //let infoLength = $("#no-more-tables > table > tbody > tr:nth-child(1)  > td").length;
                for (var i = 1; i < playerPerPage; i++) {
                    var idPlayer = $('#no-more-tables > table > tbody > tr:nth-child(' + i + ')').attr('data-playerid');
                    var linkPlayer = $('#no-more-tables > table > tbody > tr:nth-child(' + i + ') > td:nth-child(4) > a').attr('href');
                    var namePlayer = $('#no-more-tables > table > tbody > tr:nth-child(' + i + ') > td:nth-child(4) > a').text();

                    var Player = [[id = idPlayer], [nom = namePlayer], [link = linkPlayer]];
    ...
    c.queue([{
        ...
        callback: function callback(error, res, done) {
            if (error) {
                ...
                ////////////////////////////////////////////////////////////////////////////////////////
                var _idPlayer = dataLinkPlayer[a].id;
                var _lien = dataLinkPlayer[a].nom;
                ...
                var r = $('body > div.container.main > div:nth-child(3) > div.col-md-8 > div:nth-child(3) > div:nth-child(2) > div > div.panel-heading > h3 > a:nth-child(2)').attr('href');
                var idClubStr = void 0;
                if (r === undefined) {
                    var s = $('body > div.container.main > div:nth-child(3) > div.col-md-8 > div:nth-child(3) > div > div > div.panel-heading > h3 > a:nth-child(2)').text();
                    idClubStr = s.split("/");
                } else {
                    idClubStr = r.split("/");
                }
...
for (var a = 0; a < dataLinkPlayer.length; a++) {
    _loop(a);
}
